import { Community } from "./types";

const psu: Community = {
  slug: "psu",
  name: "State Media PSU",
  shortName: "Penn State",
  tagline: "The Pocket \u00b7 The Clubhouse \u00b7 Off the Mat",
  description:
    "Penn State sports coverage, athlete interviews, and officially licensed Nittany Lions gear.",
  youtubeChannel: "@StateMediaPSU",
  youtubeHandle: "@StateMediaPSU",
  storeUrl: "https://shop.gopsusports.com",
  storeName: "Penn State Team Shop",
  logo: "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Penn_State_Nittany_Lions_logo.svg/200px-Penn_State_Nittany_Lions_logo.svg.png",
  colors: {
    primary: "#041E42",
    dark: "#021633",
    darker: "#010E22",
    accent: "#1E407C",
    gradientFrom: "#041E42",
    gradientTo: "#021633",
  },
  videos: [
    {
      id: "l39ikSVptac",
      title:
        "The Transfer Portal Gamble That Changed Everything | The Clubhouse",
    },
    {
      id: "m3K-GuvkaTw",
      title:
        "What Winning a National Title Is REALLY Like | Off the Mat",
    },
    {
      id: "j5eONgvCDGk",
      title: "Inside Look at Penn State Spring Football | The Pocket",
    },
    {
      id: "2xdGMadfm-4",
      title:
        "Terry Smith Gets REAL on Penn State\u2019s New Era | The Pocket",
    },
    {
      id: "rNxhXUcs2NE",
      title:
        "Penn State Teammates Share HILARIOUS Roommate Stories | The Clubhouse",
    },
    {
      id: "pTtPkcBC6Y8",
      title:
        "Japanese Wrestling Prodigy Masanosuke Ono on His Rise to World Championship | Off the Mat",
    },
    {
      id: "nxf-XdDfaEA",
      title:
        "Breaking Down the Hardest Jobs in Baseball | The Clubhouse",
    },
    {
      id: "lmPTIe6Mp48",
      title:
        "Top 3 Biggest Spring Ball Storylines: QB Stability, Defensive Reset, WR Pressure | The Pocket",
    },
    {
      id: "pzaWqB6LZ6g",
      title:
        "Penn State Star Mitchell Mesenbrink on Wrestling, Chess, Music & Going Viral | Off the Mat",
    },
    {
      id: "Dz1ps10OTXs",
      title:
        "Spencer Barnett on Penn State Baseball, Pressure Games & the Bus Ride Story | The Clubhouse",
    },
    {
      id: "eA_LKTfsY2g",
      title:
        "Penn State\u2019s NFL Combine Was WILD: Dennis-Sutton 4.63, Drew Allar Buzz & Pro Day Scoops | The Pocket",
    },
    {
      id: "nFwHs9CqRRY",
      title:
        "Hunting, Wrestling, and Staying Calm: Levi Haines Explains His Penn State Mindset | Off The Mat",
    },
  ],
  products: [
    {
      id: "psu-hoodie-elena",
      name: "Women\u2019s Pressbox Cream Penn State Nittany Lions Elena Janise Hoodie",
      price: "$54.99",
      url: "https://shop.gopsusports.com/womens-pressbox-cream-penn-state-nittany-lions-elena-janise-hoodie/p-352214052030506432+z-8-3054301854",
      tag: "Popular",
    },
    {
      id: "psu-jersey-football",
      name: "Men\u2019s Nike #1 White Penn State Nittany Lions Football Replica Jersey",
      price: "$84.99",
      url: "https://shop.gopsusports.com/mens-nike-number-1-white-penn-state-nittany-lions-football-replica-jersey/p-59153671789592+z-817-3755718994",
      tag: "Best Seller",
    },
    {
      id: "psu-hat-trucker",
      name: "Men\u2019s Top of the World Navy Penn State Nittany Lions Trucker Adjustable Hat",
      price: "$29.99",
      url: "https://shop.gopsusports.com/mens-top-of-the-world-navy-penn-state-nittany-lions-trucker-adjustable-hat/p-15444750289027+z-9082-2218498305",
    },
    {
      id: "psu-tee-vintage",
      name: "Unisex Homefield Navy Penn State Nittany Lions Vintage Logo T-Shirt",
      price: "$39.99",
      url: "https://shop.gopsusports.com/unisex-homefield-navy-penn-state-nittany-lions-vintage-logo-t-shirt/p-27000838858738+z-9775-2403362017",
    },
    {
      id: "psu-quarter-zip",
      name: "Men\u2019s Colosseum Navy Penn State Nittany Lions Tortugas Quarter-Zip",
      price: "$64.99",
      url: "https://shop.gopsusports.com/mens-colosseum-navy-penn-state-nittany-lions-tortugas-logo-quarter-zip-jacket/p-38605836127839+z-9826-2429019270",
    },
    {
      id: "psu-sweatpants",
      name: "Men\u2019s Colosseum Navy Penn State Nittany Lions Sweatpants",
      price: "$44.99",
      url: "https://shop.gopsusports.com/mens-colosseum-navy-penn-state-nittany-lions-worlds-to-conquer-sweatpants/p-16382635054927+z-9826-1277547803",
    },
  ],
};

export default psu;
